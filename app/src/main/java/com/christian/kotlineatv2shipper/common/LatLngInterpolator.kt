...
interface LatLngInterpolator {
    ...
    class Linear:LatLngInterpolator{
        override fun interpolate(fraction: Float, a: LatLng, b: LatLng):LatLng {
            ...
            val long = (b.longitude - a.longitude)*fraction+a.longitude
    ...
    class Spherical:LatLngInterpolator{
        ...
        private fun computeAngleBetween(fromLat: Double, fromLng: Double, toLat: Double, toLng: Double): Double {
            ...
            return 2*Math.asin(Math.sqrt(Math.pow(Math.sin(dLat/2),2.0)+Math.cos(fromLat)+
                    Math.cos(toLat)*Math.pow(Math.sin(dLng/2),2.0)))
